...
from For_tests.lab72 import interpolSearch
...
class TestOnMassiv(TestCase):
    ...
    def test_for_alone(self):
        self.assertNotEqual(interpolSearch('2 3 23 13 45 67', '13' ), [None], 'Что-то неправильно работает') #!!!
    def test_for_emptiness(self):
        self.assertEqual(interpolSearch('10 2 34 57 42 12 7 9', '42'), [None], 'Что-то неправильно работает' )
...
from For_tests.lab72 import interpolSearchRec
class TestInWork(TestCase):
    def test_special_conditions_1(self): #дубликаты
        a = interpolSearchRec(list(map(int, '2 4 7 9 15 16 20 21 22 28 29'.split())), 4, 0, len(list(map(int, '2 4 7 9 15 16 20 21 22 28 29'.split())))-1)
